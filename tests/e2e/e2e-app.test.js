...
test.describe('Vue User Management E2E', () => {
   ...
   //     await expect(page).toHaveURL('http://localhost:5173/user');
    ...
    test('filter admin grid by first name john', async ({ page }) => {
        ...
        await page.locator('.ag-icon').first().click();
        ...
        //await page.waitForTimeout(1000);
        await page.getByRole('textbox', { name: 'Filter Value' }).fill('John');

        const l = '.ag-text-field-input[aria-label="Filter Value"]';

        //await page.locator(l).fill('john');


        await page.waitForTimeout(1000);
        //await expect(page.locator(l)).toHaveValue('john');
        await page.waitForTimeout(1000);
        await page.waitForTimeout(500); // Existing filter wait
        await page.waitForTimeout(1000);
        await expect(page.locator('.ag-row')).toHaveCount(1);
        await page.waitForTimeout(1000);
        await expect(page.locator('.ag-row .ag-cell:nth-child(1)')).toContainText('John');
    ...
    test('test', async ({ page }) => {
        ...
        await page.waitForTimeout(1000);
        //city
        await page.locator('div:nth-child(5) > .ag-header-cell-comp-wrapper > .ag-cell-label-container > .ag-header-icon > .ag-icon').click();
        //zip 
        await page.locator('div:nth-child(4) > .ag-header-cell-comp-wrapper > .ag-cell-label-container > .ag-header-icon > .ag-icon').click();
        //email
        await page.locator('div:nth-child(3) > .ag-header-cell-comp-wrapper > .ag-cell-label-container > .ag-header-icon > .ag-icon').click();
        await page.locator('div:nth-child(2) > .ag-header-cell-comp-wrapper > .ag-cell-label-container > .ag-header-icon > .ag-icon').click();
        //name
        await page.locator('.ag-icon').first().click();
        await page.locator('div:nth-child(5) > .ag-header-cell-comp-wrapper > .ag-cell-label-container > .ag-header-icon > .ag-icon').click();
    });
